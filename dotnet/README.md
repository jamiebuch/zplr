...
| `src/assets/*.generated.ts` | `Assets/*.cs` (generated via `D:\Cache\Temp\opencode\gen*.py`, keep diff-friendly) |
...
- SkiaSharp 4.151.1 + QRCoder 1.6.0 + ZXing.Net 0.16.9 (restored via `dotnet restore`)
...
2. Assets: re-run `python dotnet/../Cache/gen*.py` or `node scripts/generate-font-assets.mjs` then re-export to `Assets/` via the checked-in `gen_*.py` helpers.
